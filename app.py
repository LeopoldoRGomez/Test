...
def init_db():
    ...
    # (Names population removed here for the demo)
...
def get_tools_in_location(location, tool_category=None, tool_application=None, tool_specific_type=None, well=None):
    ...
    FROM tools t JOIN ToolStock ts ON t.id = ts.tool_id WHERE COALESCE(ts.{stock_column}, 0) > 0
    """
    params = []
    ...
    for _, row in df.iterrows():
        ...
        results.append({"id": row['id'], "display_name": display, "type": row['tool_type'], "quantity": int(row[stock_column]), "part_number": row['part_number'], "serial_number": row['serial_number'], "well": row['well']})
    return results
...
def generate_delivery_note_pdf(doc_number, contract_number, client, well, responsible, dispatch_date, tools_data):
    pdf = FPDF('P', 'mm', 'A4')
    pdf.add_page()
    pdf.set_font('Arial', 'B', 16); pdf.cell(0, 10, 'Delivery Note', 0, 1, 'C')
    ...
    pdf.cell(20, 10, 'Item #', 1); pdf.cell(130, 10, 'Description', 1); pdf.cell(40, 10, 'Qty', 1, 1)
    pdf.set_font('Arial', '', 9)
    for i, tool in enumerate(tools_data, 1):
        pdf.cell(20, 10, str(i), 1)
        pdf.cell(130, 10, tool['display_name'][:60], 1)
        pdf.cell(40, 10, str(tool['quantity_to_dispatch']), 1, 1)
    return pdf.output(dest='S').encode('latin1')

def generate_backload_note_pdf(doc_number, responsible, return_date, tools_data):
    pdf = FPDF('P', 'mm', 'A4')
    pdf.add_page()
    pdf.set_font('Arial', 'B', 16); pdf.cell(0, 10, 'Backload Note', 0, 1, 'C')
    pdf.set_font('Arial', '', 10)
    pdf.cell(0, 7, f'Doc #: {doc_number}', 0, 1)
    ...
    for i, tool in enumerate(tools_data, 1):
        pdf.cell(20, 10, str(i), 1)
        pdf.cell(130, 10, tool['display_name'][:60], 1)
        pdf.cell(40, 10, str(tool['quantity']), 1, 1)
    return pdf.output(dest='S').encode('latin1')
...
def search_inventory(query_term=None, sales_order_filter=None, well_filter=None):
    conn = sqlite3.connect(DB_NAME)
    query = "SELECT t.*, im.location, im.date, im.responsible, im.well, im.sales_order FROM tools t JOIN inventory_movements im ON t.id = im.tool_id"
...
def get_full_stock_report():
    ...
    query = """
        ...
        SELECT t.part_number, t.serial_number, t.description, t.specific_type, ts.warehouse_stock, ts.field_stock
        FROM tools t JOIN ToolStock ts ON t.id = ts.tool_id WHERE t.is_active = 1
    """
    df = pd.read_sql_query(query, conn)
    conn.close()
    return df
...
def get_warehouse_stock_report():
    conn = sqlite3.connect(DB_NAME)
    query = "SELECT t.part_number, t.serial_number, t.description, t.specific_type FROM tools t WHERE t.is_active = 1"
    df = pd.read_sql_query(query, conn)
    conn.close()
    return df
...
main_menu = st.sidebar.radio("Navigation", ["IN", "OUT", "Field Status", "Reports", "Query", "Administration", "Map of Wells"])
...
if main_menu == "IN":
    ...
    if not resps:
        st.warning("Setup responsibles in Administration.")
    else:
        if in_type == "Importation":
            input_mode = st.radio("Entry Mode:", ["Single Entry", "Batch Mode"], horizontal=True)
            if input_mode == "Single Entry":
                col1, col2, col3 = st.columns(3)
                sales_order = col1.text_input("Sales Order #")
                responsible = col2.selectbox("Responsible", resps)
                date = col3.date_input("Date")
                
                with st.form("add_tool_in"):
                    tool_type = st.radio("Tool Type", ["Unique_Tools", "Miscelaneous"])
                    ...
                    if st.form_submit_button("Add to List"):
                        ...
                        st.session_state.tools_to_add.append({'part_number': pn, 'serial_number': sn, 'description': desc, 'quantity': qty, 'application': app, 'specific_type': spec, 'tool_type': tool_type})
                
                if st.session_state.get('tools_to_add'):
                    st.dataframe(pd.DataFrame(st.session_state.tools_to_add))
                    if st.button("Save Full Importation"):
                        add_importation(sales_order, responsible, date.strftime('%Y-%m-%d'), st.session_state.tools_to_add)
                        st.session_state.tools_to_add = []; st.success("Saved!"); st.rerun()

        elif in_type == "Return":
            responsible = st.selectbox("Responsible", resps)
            ...
            if tools_in_field:
                ...
                doc = st.text_input("Doc Number")
                if st.button("Register Return"):
                    t_data = [t for t in tools_in_field if t['display_name'] == sel][0]
                    return_tools_batch([{'id': t_data['id'], 'quantity': qty, 'well': t_data['well']}], responsible, date.strftime('%Y-%m-%d'))
                    pdf = generate_backload_note_pdf(doc, responsible, date.strftime('%Y-%m-%d'), [{'display_name': sel, 'quantity': qty, 'part_number': t_data['part_number']}])
                    st.download_button("Download BN", pdf, "backload.pdf")

# --- OUT Section ---
elif main_menu == "OUT":
    st.header("Inventory Out to Field")
    resps = get_responsibles(); cls = get_clients(); wells = get_wells()
    if resps and cls and wells:
        ...
        if available:
            ...
            if st.button("Add to Dispatch List"):
        ...
        if st.session_state.get('dispatch_list'):
            ...
            if st.button("Confirm Dispatch"):
                ...
                st.download_button("Download DN", pdf, "delivery.pdf")
...
# --- Field Status Section ---
    ...
    if field_tools:
        ...
        sel = st.selectbox("Select Tool", [f['display_name'] for f in field_tools])
...
# --- Reports Section ---
    ...
    rep = st.selectbox("Report Type", ["Movement History", "Stock Status"])
    if rep == "Stock Status":
        st.dataframe(get_full_stock_report())
...
# --- Administration Section ---
    ...
    if pwd == "5050":
        opt = st.selectbox("Manage", ["Responsibles", "Clients", "Wells", "Equivalences", "Database"])
        if opt == "Responsibles":
            n = st.text_input("Name"); 
            if st.button("Add"): manage_responsible('add', n); st.rerun()
            st.write(get_responsibles(False))
        elif opt == "Database":
            if st.button("RESET ALL DATA"): reset_all_data(); st.warning("Data Wiped.")

# --- Map of Wells Section ---
...
# --- Query Section ---
    ...
    if st.button("Search"):
        st.dataframe(search_inventory(query_term=q))
